...
검사할 모듈의 `node_modules`에 프로젝트 패키지를 먼저 설치합니다. 각 검사 사본은 이 패키지를 링크로 참조하고, Vitest·Vite·변이 검사 도구는 검사기에 고정된 버전을 사용합니다. 검사 중 프로젝트 패키지를 자동 설치하지 않습니다. 캐시는 사본의 별도 `node_modules`에 생성합니다. Coverage의 파일·위치·소스 지문이 함수와 맞지 않으면 오류 또는 미측정으로 표시합니다.

`check`는 CRAP과 mutation을 기본으로 병렬 실행합니다. `--execution-mode sequential`로 순차 실행을 선택합니다. 실행 오류가 발생하면 다른 작업의 테스트 프로세스도 중단하고, 두 작업의 종료를 확인한 뒤 임시 폴더를 정리합니다. 두 점수 중 하나가 기준에 미달한 경우에는 나머지 검사 결과도 모아 보고합니다.

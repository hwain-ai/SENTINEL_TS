...
- **Fix** macOS 지원에서 드러난 것: (1) cli.ts 의 "직접 실행됐는가" 판정이 argv[1] 을 realpath 하지 않아 /var·/tmp 링크 아래에서는 CLI 가 아무 것도 하지 않고 종료 0 이었다. (2) 스냅샷 임시 폴더를 realpath 하지 않아 Vitest 가 보고하는 실제 경로가 "프로젝트 밖" 으로 거부됐다. (3) fcntl 을 고정 인자 함수로 선언해 Apple Silicon 에서 가변 인자 호출 규약(스택 전달)과 어긋나 잠금이 항상 실패했다. 가변 인자로 선언하고 잠금 포인터를 형식 지정 가변 인자로 넘긴다. 시험의 /usr/bin/test 호출은 existsSync 로, 잠금 파일 치환은 replaceAll 로 바꿨다.
- **Update** scripts/toolchain.py(신규)·toolchain_lock.py·toolchain.lock.json·evidence/fcntl-lock.ts·sentinel-tool/: Linux 전용 bash 실행기(bootstrap-node.sh·node.sh·npm.sh 본문)를 표준 라이브러리만 쓰는 Python 실행기로 바꿨다. 잠금 파일은 `platforms` 항목(linux-x86_64·linux-aarch64·darwin-x86_64·darwin-aarch64)에 Node 아카이브의 공식 주소·크기·SHA-256·실행 파일·npm CLI·설치 트리 지문과, 플랫폼마다 다른 node_modules 트리 지문·TypeScript 네이티브 실행 파일 경로·지문을 담는다(macOS·arm64 의 의존성 지문은 그 플랫폼에서 `describe-dependencies` 로 잰 값을 넣을 때까지 pending). 증거 커밋 잠금(fcntl)은 Linux(glibc)와 Darwin 의 struct flock 배치·명령 번호 표를 갖고 플랫폼에 맞게 고른다. 심볼릭 링크 모드는 지문에서 상수로 두어 macOS 와 Linux 의 트리 지문이 같다. wrapper 는 `python3 -I -B scripts/toolchain.py` 로 넘기고 어댑터는 자기 인터프리터로 실행기를 부른다(어댑터 0.1.1). 시험은 임시 폴더를 realpath 로 다루고 stat -c 대신 fs.stat 을 쓴다.
